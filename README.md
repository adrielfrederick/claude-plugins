...
- **`review-plan`** — runs a plan document through parallel Claude reviewer agents (Sonnet for dimension passes, Fable for holistic passes), iterating on feedback until the plan is approved or hits a max-pass cap. Pure Claude Code; no external dependencies.
